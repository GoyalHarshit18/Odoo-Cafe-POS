    ...
    <div className="min-h-screen bg-kds-background -m-4 lg:-m-6 p-4 lg:p-6">
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-4 text-sm text-kds-foreground/60">
          <span>🍳 {toCook.length} to cook</span>
          <span>🔥 {preparing.length} preparing</span>
          <span>🍽️ {completed.length} ready</span>
        </div>
      </div>
              ...
              <div className="space-y-4">
                {column.tickets.map((ticket) => (
                  <KDSTicketCard
                    key={ticket.orderId}
                    ticket={ticket}
                    onStatusChange={(status) => updateKDSStatus(ticket.orderId, status)}
                  />
                ))}
              </div>
